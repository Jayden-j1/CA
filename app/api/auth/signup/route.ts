...
// ----------------------------------------------------
// • Normalize email (trim + lowercase) before:
//     - duplicate-checking
//     - storing in the User table
//     - deriving Business.emailDomain
//   This avoids subtle bugs where "User@X.com" and "user@x.com"
//   are treated as different users and keeps auth flows consistent
//   with forgot-password + login, which also normalize.
// • Use a case-insensitive duplicate check via Prisma so we
//   cannot create the same logical email twice with different
//   casing.
//
// Pillars
// -------
// ✅ Efficiency  – simple string normalization; no extra queries
// ✅ Robustness  – case-insensitive user check; consistent identity
// ✅ Simplicity  – minimal changes, clear comments
// ✅ Ease of mgmt – fewer "my email doesn’t work" edge cases
// ✅ Security    – predictable account identity + reset flows
// ============================================================
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1) Parse the body with a narrow expected shape
    ...
    // 2) Normalize and basic validation
    //    - We trim and lowercase the email so we have a canonical form.
    //    - Name + businessName are trimmed for cleanliness.
    ...
    // Very small sanity check for email format (server-side)
    if (!email.includes("@") || email.startsWith("@") || email.endsWith("@")) {
      return NextResponse.json({ error: "Invalid email address." }, { status: 400 });
    }

    // 3) Password complexity (defense-in-depth)
    ...
    // 4) Duplicate email guard (case-insensitive)
    //
    //    Rationale:
    //    - Avoid subtle bugs where "User@Example.com" and "user@example.com"
    //      become two accounts.
    //    - This also keeps behaviour aligned with forgot-password/login,
    //      which normalise email.
    ...
    // 5) Hash password securely (bcrypt with cost 10)
    ...
    const user = await prisma.user.create({
      data: {
        name,
        email, // ✅ canonical, lowercased email
        hashedPassword,
        role,
    ...
    if (role === "BUSINESS_OWNER") {
      ...
      const emailDomain = extractEmailDomain(email); // email already normalized

      // 🔎 Detect whether the DB currently has the "emailDomain" column.
