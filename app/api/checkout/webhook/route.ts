...
// Purpose
// -------
// 1) Verify signature, accept only checkout.session.completed.
// 2) Idempotently insert Payment.
// 3) PACKAGE → set hasPaid=true for payer.
// 4) STAFF_SEAT → activate pre-created staff if metadata.newStaffId present.
//
// Robust attribution:
...
//
// Debugging:
// - Set DEBUG_STRIPE_WEBHOOK=true to log resolved attribution.
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existing) {
      // If staff seat, make sure activation happened even if payment row already exists
