...
// NOTE: This is the ONLY place we changed: we now carry `thumbnail` through.
function flattenModules(modules: any[]): any[] {
  ...
  for (const m of top) {
    ...
    const baseThumbnail = typeof m?.thumbnail === "string" ? m.thumbnail : null; // ✅ NEW
    ...
    if (Array.isArray(m?.lessons) && m.lessons.length > 0) {
      ...
      result.push({
        ...
        thumbnail: baseThumbnail, // ✅ NEW: expose thumbnail to the UI
    ...
    if (Array.isArray(m?.submodules) && m.submodules.length > 0) {
      for (const sm of sortByOrderAny(m.submodules)) {
        ...
        const subThumbnail = typeof sm?.thumbnail === "string" ? sm.thumbnail : null; // ✅ NEW
        ...
        result.push({
          ...
          description: subDesc,
          thumbnail: subThumbnail, // ✅ NEW: each flattened submodule row can also show a thumbnail
          lessons: subLessons,
        });
      }
    }
    ...
    if (
      ...
      result.push({
        ...
        title: baseTitle,
        description: baseDesc,
        thumbnail: baseThumbnail, // ✅ NEW: still expose thumbnail in the fallback row
        lessons: [
          {
            id: `${baseId}-lesson`,
            title: baseTitle,
...
export async function GET(
  ...
  try {
    ...
    const dto = {
      ...
      modules: flattenModules(doc.modules || []), // ✅ now includes `thumbnail` fields
    };
