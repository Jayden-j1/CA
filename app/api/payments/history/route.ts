...
//
// Purpose (unchanged): Return payment history respecting role-based visibility.
//
// Display-only augmentation:
// - For STAFF_SEAT rows whose Payment.description looks like "STAFF_SEAT:<email>",
//   we look up that staff user and replace the returned `user` field with the
//   staff’s identity. This keeps scoping intact (owner still owns the payment)
//   but the Billing table shows the person the seat was purchased for.
...
function parseStaffEmailFromDescription(desc: string | null | undefined): string | null {
  if (!desc) return null;
  const s = desc.trim();
  if (!s.startsWith("STAFF_SEAT:")) return null;
  const email = s.slice("STAFF_SEAT:".length).trim();
...
export async function GET(req: Request) {
  ...
  try {
    ...
    // BUSINESS_OWNER — scoped to their business
    ...
    // USER — individuals only (staff are blocked from Billing)
    if (role === "USER") {
