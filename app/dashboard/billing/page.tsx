...
//
// ✅ Update (display-only, tiny):
// - Prefer `p.displayUser` (new, beneficiary user computed server-side) when rendering
//   the "User" column. If absent, fall back to `p.user` (legacy).
// - No changes to filters, access logic, CSV export, or anything else.
//
// Why minimal UI change?
// - We keep all logic server-authoritative. The UI just picks the correct
//   user object to show for each row without changing existing behavior.
...
interface PaymentRecord {
  ...
  createdAt: string;
  // Existing payer (owner/individual) when included by API
  user?: {
    email: string;
    name: string | null;
    role: string;
  };
  // ✅ NEW optional beneficiary from API
  displayUser?: {
    email?: string;
    name?: string | null;
    role?: string;
...
function BillingPageInner() {
  ...
  const exportCSV = () => {
    ...
    const rows = payments.map((p) => {
      // ✅ Prefer beneficiary (displayUser) then fallback to payer (user)
      const u = p.displayUser || p.user;
      return [
        u?.name || "Unnamed",
        u?.email || "",
        u?.role || "N/A",
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="text-left border-b border-gray-300">
                {(payments[0]?.displayUser || payments[0]?.user) && (
