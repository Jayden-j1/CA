...
// Purpose
// -------
// Render the Billing list. NEW: If a payment is a STAFF_SEAT and its
// description encodes the beneficiary in the standard format written by the
// webhook, display that *staff* user (name/email/role) in the "User" column
// instead of the payer (business owner).
//
// What changed (surgical):
// - Added a tiny parser `parseStaffBeneficiary()` for the description pattern:
//     "Staff Seat for {NameOrEmail} <email> (ROLE)"
// - In the table cell: if `purpose === "STAFF_SEAT"` and parsing succeeds,
//   show the staff identity; otherwise fall back to `p.user`.
//
// Everything else (filters, CSV, access probe) is untouched.
...
// ────────────────────────────────────────────────────────────
// NEW: Parse beneficiary from description string
// Format produced by webhook:
//   "Staff Seat for {NameOrEmail} <email@domain> (ROLE)"
// Returns null if not matched (safe fallback to payer).
// ────────────────────────────────────────────────────────────
function parseStaffBeneficiary(desc: string | undefined) {
  if (!desc) return null;
  // Name can include spaces; email is bracketed by < >; role in ( )
      ...
      {loading ? (
        ...
        <div className="w-[90%] sm:w-[600px] md:w-[1000px] bg-white rounded-xl shadow-xl p-6 overflow-x-auto">
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {payments.map((p) => {
                // If a Staff Seat with beneficiary encoded, prefer that display
                const ben = p.purpose === "STAFF_SEAT" ? parseStaffBeneficiary(p.description) : null;
