...
import { useSearchParams } from "next/navigation"; // ✅ access ?success / ?canceled query params
...
// - Stripe redirect toasts (success/canceled)
// - Add staff form integration
function StaffDashboardContent() {
  const [staffList, setStaffList] = useState<Staff[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  // ✅ Access query params (?success=true / ?canceled=true)
  ...
  // Fetch staff once when page loads
  useEffect(() => {
    fetchStaff();
  }, []);
  ...
  useEffect(() => {
    ...
    const staffEmail = searchParams.get("staff"); // optional for more context

    if (success) {
      // ✅ Success toast with staff email (if available)
      toast.success(
        `🎉 Payment successful! ${
      ...
      // ✅ Refresh staff list so new staff appears
      fetchStaff();

      // ✅ Clean query params so toast doesn’t repeat on refresh
      window.history.replaceState(null, "", window.location.pathname);
    }

    if (canceled) {
      // ❌ Error toast for canceled payment
      toast.error(
        `❌ Payment canceled. ${
...
// ------------------------------
// - Protects route with <RequireRole>
// - Only BUSINESS_OWNER + ADMIN can access
