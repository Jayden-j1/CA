...
//
// ✅ Update (duplicate toast fix):
// - In dev (and sometimes in production during hydration), useEffect can fire
//   more than once and our success/cancel toasts were shown twice before the
//   URL was cleaned (history.replaceState).
// - We add a *one-shot guard* using sessionStorage so each redirect event
//   (identified by the current location + staff email + outcome) only triggers
//   a toast a single time. After showing it we immediately mark it as "seen"
//   and then clean the URL.
//
// Pillars:
// - Efficiency: A single string key in sessionStorage; no extra network calls.
// - Robustness: Works with StrictMode/hydration; safe if storage is disabled.
// - Simplicity: Self-contained in the tiny StaffToastHandler.
// - Ease of Management: No changes to server/APIs or other components.
// - Security: No sensitive data stored; key is ephemeral per-tab.
...
// ---------------------------------------------
// Stripe redirect toast handler (✅ duplicate-toast safe)
// ---------------------------------------------
function StaffToastHandler({ onSuccess }: { onSuccess: () => void }) {
  ...
  useEffect(() => {
    ...
    const staffEmail = (searchParams?.get("staff") ?? "").trim();

    // If neither flag is present, nothing to do.
    if (!success && !canceled) return;

    // --- One-shot guard (prevents duplicate toasts) --------------------------
    // We use sessionStorage so the guard only affects this browser tab
    // and only for the current navigation. The key includes:
    // - path (so different pages don’t collide),
    // - outcome (success|canceled),
    // - staff email (so multiple sequential adds are independent).
    const path = typeof window !== "undefined" ? window.location.pathname : "/dashboard/staff";
    const key = `staff:toast:${path}:${success ? "success" : "canceled"}:${staffEmail || "unknown"}`;

    try {
      const alreadyShown = typeof window !== "undefined" && sessionStorage.getItem(key) === "1";
      if (alreadyShown) {
        // We've already shown this exact toast once → just clean the URL if needed and bail.
      ...
      // Mark as shown BEFORE displaying toast to be safe against rapid re-renders.
      if (typeof window !== "undefined") {
        sessionStorage.setItem(key, "1");
      }
    } catch {
      // If storage is unavailable, we still proceed (worst case: double toast in rare envs).
    }
    // ------------------------------------------------------------------------
    ...
    // Clean the query string so a manual refresh doesn’t retrigger the toast.
    if (typeof window !== "undefined") {
      window.history.replaceState(null, "", path);
    }
