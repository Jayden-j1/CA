...
//
// This patch (surgical):
// - Explicitly set `type="button"` on the Submit button to prevent accidental
//   parent-form submission (default <button> is "submit"), which can cause
//   "stuck" states in some hosting environments.
// - Keep everything else exactly the same.
// - Add small a11y helpers (`aria-busy`, `aria-live`) for clearer state.
//
// Pillars:
// - Simplicity: minimal props, no local correctness state.
// - Robustness: safe checks around indices and explicit button type.
// - Accessibility: fieldset/legend, labeled radios, clear live/ busy states.
...
const QuizCard: React.FC<QuizCardProps> = ({
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200" aria-busy={revealed}>
      ...
      {/* Status line for screen readers so users hear that submission happened */}
      <p className="sr-only" aria-live="polite">
        {revealed ? "Answers submitted. Advancing…" : ""}
      </p>

      <button
        type="button"        // ✅ Critical: avoid accidental parent form submission
        onClick={onSubmit}
      ...
      >
        {revealed ? "Submitted — continuing…" : "Submit"}
      </button>
    </div>
  );
};
