 ...
 * --------------------
 * Now fully wired to POST to /api/contact with:
 * - client-side validation for email + basic required fields
 * - graceful handling of success, validation errors, and rate limits
 * - minimal UI changes (structure and field names unchanged)
 *
 * What changed (behavioural):
 * - We added an onSubmit handler that:
 *   • prevents default browser submission,
 *   • sends a JSON payload to /api/contact,
 *   • handles loading state + success/error messages,
 *   • resets the form on success.
 *
 * What did NOT change:
 * - Field IDs, names, and labels (backwards compatible).
 * - The radio button wiring for preferredContact (still uses `selected` state).
 * - Mobile/email validation styling remains the same.
 */
...
export default function ContactFormComponent() {
  ...
  // Tracks which radio button is selected (UNCHANGED).
  const [selected, setSelected] = useState<'email' | 'mobile' | 'textMessage' | ''>('');

  // Stores the user's mobile number input (restricted to 10 digits) (UNCHANGED).
  const [mobileNumber, setMobileNumber] = useState('');

  // Stores the user's email input (UNCHANGED).
  const [email, setEmail] = useState('');

  // ---------------------- New state for UX ----------------------

  // Tracks whether the form is currently submitting (prevents double-clicks).
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Simple status object for user feedback (success / error banner).
  ...
  // Handler for radio button change (UNCHANGED logic).
   ...
   * - Performs light client-side validation (email + message presence).
   * - Sends JSON to /api/contact and interprets the response status.
   * - Updates UI with success or error messages.
   */
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();

    // Avoid duplicate submissions if the user clicks multiple times quickly.
    ...
    // Basic client-side checks (server will also validate).
    if (!name) {
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        // We send the exact fields the API expects.
        body: JSON.stringify({
          name,
          mobileNumber,
          phoneNumber,
          email,
          message,
          preferredContact: selected || formData.get("preferredContact") || undefined,
        }),
      });
      ...
      // For non-200 responses, try to read a JSON error message.
  ...
  return (
    /**
     * Layout:
     * - We keep the same white card and spacing so the visual design remains familiar.
     * - The only functional change is that the <form> now has an onSubmit handler.
     */
    <form
      action="#"
      method="post"
    ...
    >
      {/* Name Input (logic unchanged, now read via FormData in handleSubmit) */}
      <div>
      ...
      {/* Mobile Input (logic unchanged — numeric-only and 10 digit max) */}
      <div>
      ...
      {/* Phone Input (logic unchanged: separate phone field, read via FormData) */}
      <div>
      ...
      {/* Email Input (logic unchanged — still uses emailRegex + suggestion UI) */}
      <div>
      ...
      {/* Message Textarea (logic unchanged, value read via FormData) */}
      <div>
      ...
      {/* Preferred contact method — Radios (UI + wiring unchanged) */}
      ...
      {/* Status banner (NEW, non-intrusive) */}
      {status.type === 'success' && (
      ...
      {/* Submit button (same styling, now shows "Sending..." when busy) */}
      <button
        type="submit"
        id="submit"
        value="clientContact"
        disabled={isSubmitting}
        className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-75 disabled:cursor-not-allowed text-white font-semibold py-2.5 rounded transition-colors"
      >
