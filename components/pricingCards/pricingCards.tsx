...
// - Keeps UX clear: Guests must sign up before paying, members can pay directly.
//
// Dependencies:
// - next-auth (useSession) to detect login state.
// - CheckoutButton (reusable payment component).

"use client";

import React from "react";
import { useSession } from "next-auth/react"; // ✅ to check login state
...
const PricingCardSection: React.FC = () => {
  // ✅ Session tells us if the user is logged in
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-8 max-w-7xl mx-auto">
          {pricingCards.map(({ name, price, services }, index) => {
            const packageType = name.includes("Individual")
              ? "individual"
              : "business";
